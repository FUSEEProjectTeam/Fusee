...
namespace Fusee.Math.Core
{
    ...
    public static class M
    {
        ...
        /// <param name="zPosition">Desired z coordinate in world space</param>
        ...
        /// <returns></returns>
        public static float3 ScreenPointToWorld(float2 windowCoordIn, float zPosition, float4x4 invProjection, float4x4 invView, int windowWidth, int windowHeight)
        {
            var pickPosClip = (windowCoordIn * new float2(2.0f / windowWidth, -2.0f / windowHeight)) + new float2(-1, 1);
            var vec = new float4(pickPosClip.x, pickPosClip.y, zPosition, 1f);
