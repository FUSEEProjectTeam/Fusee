...
namespace Fusee.Math.Geometry2D
{
	...
	public struct Ray : ICloneable, ISerializable
	{
		...
		{
			_origin		= (Vector2F)info.GetValue("Origin", typeof(Vector2F));
			_direction	= (Vector2F)info.GetValue("Direction", typeof(Vector2F));
		}
		#endregion
		...
		{
			info.AddValue("Origin", _origin, typeof(Vector2F));
			info.AddValue("Direction", _direction, typeof(Vector2F));
		}
		#endregion
		...
		public static Ray Parse(string s)
		{
			...
			{
				return new Ray(
					Vector2F.Parse(m.Result("${origin}")),
					Vector2F.Parse(m.Result("${direction}"))
					);
			}
			else
			{
