...
void main()
{
	...
	{		
		...
		if(currentTileX == 0.0)
		{
			...
			outColor = vec4(texture(DiffuseTexture, vec2(uvX , uvY)) * DiffuseMix) * Albedo *  max(dot(N, L), 0.0);
		}
		//Last tile
		...
		outColor = vec4(texture(DiffuseTexture,vUV) * DiffuseMix)* Albedo *  max(dot(N, L), 0.0);	
}