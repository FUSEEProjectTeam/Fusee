...
namespace Fusee.Engine.Imp.Graphics.Blazor
{
    ...
    public class GamePadDeviceImp : IInputDeviceImp
    {
        ...
        public string Id
        {
            get
            {
                ...
                using (IJSInProcessObjectReference navigator = ((IJSInProcessRuntime)runtime).GetGlobalObject<IJSInProcessObjectReference>("navigator"))
                {
                    IJSInProcessObjectReference[] Gamepads = navigator.Invoke<IJSInProcessObjectReference[]>("getGamepads");
                    Diagnostics.Debug($"Trying to connect to {Gamepads.Length} gamepads:");

                    for (int i = 0; i < Gamepads.Length; i++)
                    {
                        using IJSInProcessObjectReference Gamepad = Gamepads.GetObjectProperty<IJSInProcessObjectReference>(i.ToString());
                        ...
                        if (Gamepad != null)
                        {
                            string id = Gamepad.GetObjectProperty<string>("id");
                            Diagnostics.Debug($"Gamepad {i}: ID={id}");
                        }
                    }
                    using (IJSInProcessObjectReference Gamepad = Gamepads.GetObjectProperty<IJSInProcessObjectReference>(DeviceID.ToString()))
                    {
                        if (Gamepad != null)
                        {
                            string _index = Gamepad.GetObjectProperty<string>("id");
                            {
                                Index = _index;
                            }
                        }
    ...
    public class KeyboardDeviceImp : IInputDeviceImp
    {
        ...
        internal KeyboardDeviceImp(IJSObjectReference _, IJSRuntime runtime)
        {
            ...
            System.Collections.IEnumerator enumName = Enum.GetNames(typeof(KeyCodes)).GetEnumerator();
            System.Collections.IEnumerator enumValue = Enum.GetValues(typeof(KeyCodes)).GetEnumerator();
        ...
        public IEnumerable<ButtonImpDescription> ButtonImpDesc
        {
            get
            {
                foreach (ButtonDescription key in _keyDescriptions.Values)
                {
    ...
    public class MouseDeviceImp : IInputDeviceImp
    {
        ...
        {
            IJSInProcessObjectReference w = _runtime.GetGlobalObject<IJSInProcessObjectReference>("window");
            int res = w.GetObjectProperty<int>("innerWidth");
            return res;
        }
        ...
        {
            IJSInProcessObjectReference w = _runtime.GetGlobalObject<IJSInProcessObjectReference>("window");
            int res = w.GetObjectProperty<int>("innerHeight");
            return res;
        }
        #endregion
    ...
    public class TouchDeviceImp : IInputDeviceImp
    {
        ...
        private void ConnectCanvasEvents()
        {
            ((IJSInProcessObjectReference)_canvas).InvokeVoid("addEventListener", new object[] { "touchstart", new Action<IJSInProcessObjectReference>(
                (IJSInProcessObjectReference evt) => {
                    ...
                    IJSInProcessObjectReference[] touches = evt.GetObjectProperty<IJSInProcessObjectReference[]>("changedTouches"); int nTouches = touches.Length;
                        for (int i = 0; i < nTouches; i++)
                        {
                            using IJSInProcessObjectReference touch = touches.GetObjectProperty<IJSInProcessObjectReference>(i.ToString());
                            OnCanvasTouchStart(
        ...
        private float GetWindowWidth()
        {
            using IJSInProcessObjectReference w = _runtime.GetGlobalObject<IJSInProcessObjectReference>("window");
