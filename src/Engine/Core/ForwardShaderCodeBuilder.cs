    ...
    public class ForwardShaderCodeBuilder
    {
        ...
        private void ApplyLightMethod(StringBuilder vs)
        {
            ...
            else if (_lightningCalculationMethod == LightningCalculationMethod.BLINN_PHONG)
            {
                ...
                vs.Append("     if(light.lightType == 0) // PointLight\n");
                vs.Append("     {");
                vs.Append($"           {PointLightCalculation()}\n");
                vs.Append("     }\n");
                vs.Append("     else if(light.lightType == 1) // ParallelLight\n");
                vs.Append("     {");
                vs.Append($"            {ParallelLightCalculation()}\n");
                vs.Append("     }\n");
                vs.Append("     else if(light.lightType == 2) // SpotLight\n");
                vs.Append("     {");
        ...
        private string SpecularLightingMethod()
        {
            ...
            outputString += "   }\n";
            outputString += $"  return ({SpecularColorName} * {SpecularIntensityName} * light.intensities * specularTerm) * 1000.0;\n";
            outputString += "}\n";
        ...
        private string ParallelLightCalculation()
        {
            ...
            outputString += "vec3 o_toLight = normalize(mat3(FUSEE_MV) *light.position.xyz - surfacePos.xyz);\n";
            outputString += "vec3 o_toCamera = normalize(mat3(FUSEE_MV) * vViewDir - surfacePos.xyz);\n";
        ...
        private string SpotLightCalculation()
        {
            ...
            outputString += "       vec3 Normal = vNormal;\n";
            //vec3 surfaceWorldPosition = (u_world * a_position).xyz;
            outputString += "       vec3 v_surfaceToLight = normalize(light.position.xyz - surfacePos.xyz);\n";
            outputString += "       float lightDot = dot(Normal, light.position.xyz);\n";
            ...
            outputString += "       float lightToSurfaceAngle = degrees(acos(dot(-v_surfaceToLight, normalize(light.coneDirection))));\n";
            outputString += "       if (lightToSurfaceAngle > light.coneAngle)\n";
            outputString += "       {\n";
            outputString += "       attenuation = 0.0;\n";
            outputString += "       }\n";
        ...
        private void PSBody(StringBuilder vs)
        {
            ...
            vs.Append("{\n");
                ...
                vs.Append("    {\n");
                    vs.Append("         result += ApplyLight(allLights[i]);\n");
                vs.Append("    }\n");
