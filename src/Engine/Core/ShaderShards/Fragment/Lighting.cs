...
namespace Fusee.Engine.Core.ShaderShards.Fragment
{
    ...
    public static class Lighting
    {
        ...
        /// This may become an issue with more advanced lighting calculations. See [Akenine-M�ller 2018, Real-Time-Rendering 4th Edition, p. 308 - 316].
        /// </summary>
        ...
        public static string ApplyLightForward(LightingSetupFlags setup)
        {
            ...
            else if (setup.HasFlag(LightingSetupFlags.BRDF))
            {
                ...
                methodBody.Add($"Idif = diffuseLighting(surfOut.albedo.rgb, NdotL, NdotV, LdotH, surfOut.{SurfaceOut.Roughness.Item2}, surfOut.{SurfaceOut.Subsurface.Item2}, surfOut.subsurfaceColor.rgb);");
        ...
        public static string ApplyLightDeferred(Light lc, bool isCascaded = false, int numberOfCascades = 0, bool debugCascades = false)
        {
            ...
            "{",
                ...
                "vec3 diff = diffuseLighting(albedo.rgb, NdotL, NdotV, LdotH, roughness, subsurface, subsurfaceColor);",
