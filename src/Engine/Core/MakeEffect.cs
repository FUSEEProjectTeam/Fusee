...
namespace Fusee.Engine.Core
{
    ...
    public static class MakeEffect
    {
        ...
        /// <param name="specularStrength">The resulting effects specular intensity.</param>
        /// <returns>A ShaderEffect ready to use as a component in scene graphs.</returns>
        public static SurfaceEffect FromDiffuseSpecularTexture(float4 albedoColor, float shininess, string texName, float albedo, float specularStrength = 0.5f)
        {
            var input = new TextureInput()
            {
                ...
                SpecularStrength = specularStrength,
                AlbedoMix = albedo,
                AlbedoTex = AssetStorage.Get<Texture>(texName)
            };
        ...
        private static string CreateVertexShader(Weight wc, EffectProps effectProps)
        {
