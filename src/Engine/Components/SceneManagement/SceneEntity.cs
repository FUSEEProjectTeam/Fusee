...
{
    /// <summary>
    ///     The SceneEntity class can contain components that do different tasks during runtime in order to create a scene.
    ///     Every SceneEntity automatically has a transform component which is used for the 3D orientation.
    ///     It is required to give a SceneEntity a name in order to use the search algorithms of <see cref="SceneManager" />
    ///     during runtime.
    /// </summary>
    public class SceneEntity
    {
        ...
        /// </summary>
        private readonly List<SceneEntity> _childSceneEntities;

        /// <summary>
        ///     The components of the current SceneEntity.
        /// </summary>
        private readonly List<Component> _childComponents;
        ...
        /// <param name="renderer">The _renderer.</param>
        public SceneEntity(string name, ActionCode action, Material material, Renderer renderer)
        {
            _childComponents = new List<Component>();
            _childSceneEntities = new List<SceneEntity>();
            Name = name;
        ...
        /// <param name="material">The material.</param>
        /// <param name="renderer">The _renderer.</param>
        public SceneEntity(string name, Material material, Renderer renderer)
        {
            _childComponents = new List<Component>();
            _childSceneEntities = new List<SceneEntity>();
            Name = name;
        ...
        /// <param name="name">The _name.</param>
        /// <param name="action">The action.</param>
        public SceneEntity(string name, ActionCode action)
        {
            _childComponents = new List<Component>();
            _childSceneEntities = new List<SceneEntity>();
        ...
        /// </summary>
        /// <param name="sceneVisitorRendering">
        ///     The scene visitor rendering that grabs the informations from the SceneEntity that
        ...
        {
            var rendererComp = component as Renderer;
            if (rendererComp != null) // TODO sorting addition process
            {
                if (Renderer == null)
                {
                    _childComponents.Add(rendererComp);
        ...
        /// </summary>
        /// <typeparam name="T">
        ///     This is the type of the Component. The type in this case can be any derivate of the Component
        ///     class, e.g. <see cref="SceneManagement.Renderer" />
        /// </typeparam>
        /// <returns>On successful search a list of Components of specified type is returned.</returns>
        public T[] GetComponents<T>()
        {
            return (from childComponent in _childComponents where childComponent.GetType() == typeof (T) select (T) (object) childComponent).ToArray();
        }

        /// <summary>
        ///     Finds a SceneEntity in the scene. This method is an access point to FindSceneEntity method inside
