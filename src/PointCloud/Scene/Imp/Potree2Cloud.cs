...
namespace Fusee.PointCloud.Scene.Imp
{
    ...
    public class Potree2Cloud : IPointCloudImp, IDisposable
    {
                ...
                if (guid != null)
                {
                    switch (PointCloudLoader.PtAccessor.PointType)
                    {
                        ...
                        case PointType.Undefined:
                            throw new ArgumentException();
                        case PointType.PosD3:
                            meshes = TryGetMeshesFromCache<PosD3>(guid);
                            break;
                        case PointType.PosD3ColF3InUs:
                            meshes = TryGetMeshesFromCache<PosD3ColF3InUs>(guid);
                            break;
                        case PointType.PosD3InUs:
                            meshes = TryGetMeshesFromCache<PosD3InUs>(guid);
                            break;
                        case PointType.PosD3ColF3:
                            meshes = TryGetMeshesFromCache<PosD3ColF3>(guid);
                            break;
                        case PointType.PosD3LblB:
                            meshes = TryGetMeshesFromCache<PosD3LblB>(guid);
                            break;
                        case PointType.PosD3NorF3ColF3InUs:
                            meshes = TryGetMeshesFromCache<PosD3NorF3ColF3InUs>(guid);
                            break;
                        case PointType.PosD3NorF3InUs:
                            meshes = TryGetMeshesFromCache<PosD3NorF3InUs>(guid);
                            break;
                        case PointType.PosD3NorF3ColF3:
                            meshes = TryGetMeshesFromCache<PosD3NorF3ColF3>(guid);
                            break;
                        case PointType.PosD3ColF3LblB:
                            meshes = TryGetMeshesFromCache<PosD3ColF3LblB>(guid);
                            break;
                        case PointType.PosD3ColF3InUsLblB:
                            meshes = TryGetMeshesFromCache<PosD3ColF3InUsLblB>(guid);
                            break;
                    }
        ...
        private IEnumerable<GpuMesh> TryGetMeshesFromCache<TPoint>(string guid)
        {
            IEnumerable<GpuMesh> meshes;
            switch (PointCloudLoader.PtAccessor.PointType)
            {
                ...
                case PointType.PosD3:
                    {
                        ...
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3ColF3InUs:
                    {
                        if (!((PointCloudLoader<PosD3ColF3InUs>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3ColF3InUs>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3InUs:
                    {
                        if (!((PointCloudLoader<PosD3InUs>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3InUs>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3ColF3:
                    {
                        if (!((PointCloudLoader<PosD3ColF3>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3ColF3>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3LblB:
                    {
                        if (!((PointCloudLoader<PosD3LblB>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3LblB>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3NorF3ColF3InUs:
                    {
                        if (!((PointCloudLoader<PosD3NorF3ColF3InUs>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3NorF3ColF3InUs>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3NorF3InUs:
                    {
                        if (!((PointCloudLoader<PosD3NorF3InUs>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3NorF3InUs>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3NorF3ColF3:
                    {
                        if (!((PointCloudLoader<PosD3NorF3ColF3>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3NorF3ColF3>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3ColF3LblB:
                    {
                        if (!((PointCloudLoader<PosD3ColF3LblB>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3ColF3LblB>(points, points.Length), out meshes);
                        break;
                    }
                case PointType.PosD3ColF3InUsLblB:
                    {
                        if (!((PointCloudLoader<PosD3ColF3InUsLblB>)PointCloudLoader).PointCache.TryGetValue(guid, out var points)) return null;
                        _meshCache.AddOrUpdate(guid, new GpuMeshFromPointsEventArgs<PosD3ColF3InUsLblB>(points, points.Length), out meshes);
                        break;
                    }
            }
