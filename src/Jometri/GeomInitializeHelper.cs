...
namespace Fusee.Jometri
{
    ...
    public static class GeomInitializeHelper
    {
        ...
        internal static void CreateHalfEdgesForGeometry(this Geometry geometry, IEnumerable<PolyBoundary> outlines)
        {
            ...
            {
                var boundaryHalfEdges = CreateHalfEdgesForBoundary(geometry, o);
        ...
        private static IEnumerable<BoundaryEdge> CreateHalfEdgesForBoundary(Geometry geometry, PolyBoundary outline)
        {
            var outlineVerts = OutlineVertices(geometry, outline);
            var boundaryEdges = BoundaryEdges(geometry, outlineVerts, outline);
            ...
            for (var i = boundaryEdges.Count - 1; i > -1; i--)
            {
                ...
                if (!IsEdgeExisting(geometry, bEdge.HalfEdge, boundaryEdges, out int existingHeHandle))
                ...
                var face = geometry.GetFaceByHandle(bEdge.HalfEdge.IncidentFace);
                ...
                var unboundedFace = geometry.DictFaces[1];
                for (var k = 0; k < unboundedFace.InnerHalfEdges.Count; k++)
                {
                    ...
                    var nextHe = geometry.GetHalfEdgeByHandle(heHandle).NextHalfEdge;

                    unboundedFace.InnerHalfEdges[k] = nextHe;
                    geometry.DictFaces[1] = unboundedFace;
                    break;
                }

                var existingHe = geometry.GetHalfEdgeByHandle(existingHeHandle);
                ...
                SetPrevAndNextToExistingHalfEdge(geometry, bEdge, existingHeHandle, boundaryEdges, halfEdgesToUpdate);
            ...
            {
                geometry.ReplaceHalfEdge(he);
            }
            return boundaryEdges;
        }

        private static int AddFace(Geometry geometry, int firstHalfEdge, out Face face)
        {
            face = new Face(geometry.DictFaces.Count + 1, firstHalfEdge);
            return face.Handle;
        }

        private static Vertex CreateOrFindVertex(Geometry geometry, float3 pointCoord, out bool isOldVertex, int handle)
        {
            int vertHandle;
            Vertex vert;

            //Check if a Vertex already exists and assign it to the HalfEdge instead of creating a new.
            if (geometry.DictVertices.Count != 0)
            {
                foreach (var v in geometry.GetAllVertices())
                {
        ...
        private static List<KeyValuePair<Vertex, bool>> OutlineVertices(Geometry geometry, PolyBoundary polyBoundary)
        {
            var outlineVerts = new List<KeyValuePair<Vertex, bool>>();

            var handle = 0;
            if (geometry.DictVertices.Count != 0)
                handle = geometry.DictVertices.Count;

            foreach (var coord in polyBoundary.Points)
            {
                handle++;
                var vert = CreateOrFindVertex(geometry, coord, out bool isOldVert, handle);
                outlineVerts.Add(new KeyValuePair<Vertex, bool>(vert, isOldVert));
            }
            return outlineVerts;
        }

        private static List<BoundaryEdge> BoundaryEdges(Geometry geometry, IList<KeyValuePair<Vertex, bool>> outlineVerts,
            PolyBoundary polyBoundary)
        {
            ...
            if (geometry.DictHalfEdges.Count != 0)
                halfEdgeHandle = geometry.DictHalfEdges.Count;
            ...
            for (var j = 0; j < outlineVerts.Count; j++)
            {
                ...
                if (!currentVert.Value)
                {
                    ...
                    geometry.DictVertices.Add(vert.Handle, vert);
                }
                ...
                if (polyBoundary.IsOuter)
                {
                    ...
                    {
                        faceHandle = AddFace(geometry, halfEdge.Handle, out Face face);
                        geometry.DictFaces.Add(face.Handle, face);
                    }
                }
                else
                {
                    if (j == 0)
                    {
                        var lastFace = geometry.DictFaces[geometry.DictFaces.Keys.Max()];
                        lastFace.InnerHalfEdges.Add(halfEdge.Handle);
                    }
                    faceHandle = geometry.DictFaces.Last().Value.Handle;
                }
                ...
                {
                    var unboundFace = geometry.DictFaces[1];

                    if (j == 0)
                    {
                        unboundFace.InnerHalfEdges.Add(twinHalfEdge.Handle);
                        geometry.DictFaces[1] = unboundFace;
                    }
                }
        ...
        private static void SetPrevAndNextToExistingHalfEdge(Geometry geometry, BoundaryEdge bEdge, int existingHeHandle,
            IList<BoundaryEdge> boundaryEdges, ICollection<HalfEdge> halfEdgesToUpdate)
        {
            var existingHe = geometry.GetHalfEdgeByHandle(existingHeHandle);
            var existingHeNext = geometry.GetHalfEdgeByHandle(existingHe.NextHalfEdge);
            var existingHePrev = geometry.GetHalfEdgeByHandle(existingHe.PrevHalfEdge);
        ...
        private static bool IsEdgeExisting(Geometry geometry, HalfEdge halfEdge, IEnumerable<BoundaryEdge> boundaryEdges, out int existingHeHandle)
        {
            ...
            {
                var he = heHandle;
                var oldHeTargetVert = geometry.GetHalfEdgeByHandle(he.NextHalfEdge).OriginVertex;
